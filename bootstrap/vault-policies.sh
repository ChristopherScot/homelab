...
# CNPG backup credentials, one role per namespace holding a Postgres
# cluster that backs up to S3.
#
# Deliberately NOT folded into an app's own role. A service's policy is
# generated by homelabctl from its config.yaml, so a hand-edit here
# would be overwritten on the next `homelabctl vault --apply`.
#
# ONE role and ONE policy for every namespace, replacing the loop that
# used to write a role per namespace. The policy is TEMPLATED on the
# calling namespace:
#
#   aws/static-creds/{{...metadata.service_account_namespace}}
#
# Vault substitutes the namespace from the authenticated token, so a
# pokedex token reads aws/static-creds/pokedex and nothing else. Adding
# a database to the fleet is now a namespace in the list below plus a
# cnpg-backup ServiceAccount in it - no new role, no new policy.
#
# The credentials are AWS STATIC ROLES, not dynamic ones. A dynamic role
# mints a new IAM user on every read, and External Secrets re-reads on a
# schedule without owning leases - so every refresh orphaned the
# previous user and left the cluster holding a revoked key. A static
# role owns one long-lived user per service and rotates its key on a
# schedule, which is what makes repeated reads idempotent.
#
# Each user is scoped to its own prefix
# (s3://cscot-homelab-pg-backups/<service>/*) and carries the
# cnpg-backup-boundary permissions boundary, so even a policy mistake
# cannot reach another service's backups or anything outside the bucket.
# The accessor is per-Vault-instance and changes if the auth method is
# re-enabled, so it is read rather than hardcoded. A stale accessor
# makes the template silently resolve to an empty namespace, which
# denies everything.
K8S_AUTH_ACCESSOR=$(vault auth list -format=json | jq -r '."kubernetes/".accessor')

vault policy write cnpg-backup - <<EOF
path "aws/static-creds/{{identity.entity.aliases.${K8S_AUTH_ACCESSOR}.metadata.service_account_namespace}}" {
  capabilities = ["read"]
}
EOF

vault write auth/kubernetes/role/cnpg-backup \
  bound_service_account_names="cnpg-backup" \
  bound_service_account_namespaces="pokedex,shlink,monitoring,authelia,paperless,vaultwarden" \
  policies="cnpg-backup" \
